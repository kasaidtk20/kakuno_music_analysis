...
#ユーザー行をリセットする
prex = pd.read_csv('../csv/data_preset_x.csv', encoding = 'shift_jis')
prex = prex.head(16)
prex.to_csv('../csv/data_preset_x.csv', encoding = 'shift_jis', index=False)
...
for i in range(8):
    ...
    to_append = f'{paramsU.user_music}_{i} {np.mean(chroma_stft)} {np.mean(rmse)} {np.mean(spec_cent)} {np.mean(spec_bw)} {np.mean(rolloff)} {np.mean(zcr)} {np.mean(mel_spec)}'
    to_append = to_append.split()

    Umusic.close()

    prex = open('../csv/data_preset_x.csv', 'a', newline='')
    with prex:
